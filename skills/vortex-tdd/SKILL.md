...
Esta skill é o padrão de execução de código de qualquer task do time de agentes da Vortex. Ela vem **pré-carregada** no frontmatter de `vortex-implementador`, `vortex-dev-backend`, `vortex-dev-frontend` e `vortex-dev-dados` (campo `skills:`) — eles não escolhem carregá-la, ela já está no contexto quando a task começa. `vortex-revisor` cobra evidência de que o ciclo abaixo foi seguido; `vortex-subagentes` é quem despacha a task que aciona esta skill.
...
| Teste sem asserção real, tautológico ou acoplado à implementação | **`vortex-revisor`**, na revisão por task |

Os gates mecânicos são o **piso**; o `vortex-revisor` é o **teto**. Nenhum dos dois sozinho fecha — e no meio, entre um e outro, está a disciplina que esta skill descreve. É por isso que ela é escrita como lei e não como sugestão.
