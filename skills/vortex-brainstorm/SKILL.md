---
name: vortex-brainstorm
description: "Skill do sistema de agentes da Vortex. Use SEMPRE antes de qualquer trabalho criativo — criar features, construir componentes, adicionar funcionalidade ou alterar comportamento existente. Explora intenção do usuário, requisitos e design antes de qualquer implementação. Gatilhos: 'quero criar/adicionar/mudar algo no produto', 'vamos construir uma feature', início de qualquer projeto ou pedido de mudança de comportamento, mesmo que pareça simples."
---

# Vortex Brainstorm — Transformar Ideias em Design

Skill própria do sistema de agentes da Vortex para conduzir, em diálogo natural e colaborativo, a transformação de uma ideia em um design/spec pronto para virar plano de implementação.
...
3. **Propor 2-3 abordagens** — com trade-offs e sua recomendação (`vortex-pesquisador` quando a comparação depender de fato externo)
4. **Apresentar o design** — em seções proporcionais à complexidade, com aprovação do usuário após cada seção
5. **Escrever o documento de design** — salvar em `.vortex/sdd/specs/YYYY-MM-DD-<topico>-design.md` e commitar
...
8. **Transição para implementação** — invocar a skill `vortex-plano` para criar o plano de implementação

## Fluxo do processo

```dot
digraph vortex_brainstorm {
    ...
    "Invocar skill vortex-plano" [shape=doublecircle];
    ...
    "Usuário revisou o spec?" -> "Invocar skill vortex-plano" [label="aprovado"];
}
```

**O estado terminal é invocar `vortex-plano`.** NÃO invoque skills de implementação, agentes de código ou qualquer outra skill de design. A ÚNICA skill invocada depois do brainstorm é `vortex-plano`.
...
- Se a escolha **depender de fato externo ao repositório** (maturidade e manutenção de uma
  ...
  despache o agente `vortex-pesquisador` com as perguntas exatas e aguarde o relatório **em arquivo**.
...
- Unidades pequenas e bem delimitadas também facilitam o trabalho dos agentes Vortex — código que cabe no contexto de uma vez é mais fácil de raciocinar, e edições ficam mais confiáveis quando os arquivos são focados. Um arquivo crescendo demais costuma ser sinal de que está fazendo coisa demais.
...
- Escreva o design validado (spec) em `<VORTEX_DOCS_PATH>/specs/YYYY-MM-DD-<topico>-design.md`
  (default: `.vortex/sdd/specs/`)
...
- Invoque a skill `vortex-plano` para criar um plano de implementação detalhado
- NÃO invoque nenhuma outra skill. `vortex-plano` é o próximo passo.
...
> Os caminhos `.vortex/sdd/…` são o default. Se o projeto configurou `VORTEX_DOCS_PATH` ou `VORTEX_LEDGER_PATH` (na instalação do plugin ou em `.vortex/config.json`), use o caminho configurado — o default vale só quando não há configuração.
