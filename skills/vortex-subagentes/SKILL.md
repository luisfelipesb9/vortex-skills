...
# Vortex — Desenvolvimento Orientado por Subagentes

Executa um plano despachando um subagente implementador novo por task (o
`vortex-implementador`), seguido de uma revisão por task (spec + qualidade,
feita pelo `vortex-revisor`), e termina com uma revisão ampla de toda a branch.
...
Se não há plano ainda, use `vortex-brainstorm` e depois `vortex-plano` antes de
...
1. Despachar o agente declarado no campo **Agente** da task, com o brief
   dela. Plano antigo, sem esse campo: `vortex-implementador`. O campo viaja no
...
4. Assim que o status for CONCLUIDO (ou CONCLUIDO_COM_RESSALVAS resolvido), monte o
   diff da task e despache o subagente **revisor** (o agent `vortex-revisor`)
   com esse diff.
...
1. **Revisão final de branch.** Despache o `vortex-revisor` com o range
...
6. **Confirmar com o humano — uma linha, não um interrogatório.** Mostre
   ...
   **Se você estiver em contexto forkado** (foi despachado por
   `/vortex-executar`), a pergunta não chega ao humano de forma confiável:
   **não pergunte.** Pare e devolva `Branch <x> pronta, revisão final limpa,
   N Menores pendentes. Rode /vortex-pr para abrir.`
...
O `vortex-implementador` reporta um dos quatro status:

- **CONCLUIDO:** monte o pacote de revisão (commit list + diff da task, veja
  "Como entregar artefatos" abaixo) e despache o `vortex-revisor`.
...
O `vortex-revisor` pode reportar itens que exigem contexto que não está no
...
- **Brief da task:** antes de despachar um implementador, extraia o texto
  ...
  `.vortex/sdd/tasks/task-N-brief.md` e componha o despacho para que o brief seja a fonte
...
- **Arquivo de relatório:** nomeie o relatório do implementador a partir do
  brief (brief `.vortex/sdd/tasks/task-N-brief.md` → relatório
  `.vortex/sdd/tasks/task-N-report.md`) e informe
...
- **Entradas do revisor:** o `vortex-revisor` recebe três caminhos — o mesmo
...
- No início da execução, verifique se já existe um ledger de progresso
  (o caminho de `VORTEX_LEDGER_PATH`; default `.vortex/sdd/progress.md`). Tasks lá marcadas como
...
[Monta o pacote de diff, despacha vortex-revisor com o caminho]
vortex-revisor: Spec ✅ - todos os requisitos atendidos, nada extra.
...
[Extrai o brief da Task 2; despacha vortex-implementador com brief + caminho
 de relatório + contexto]
...
[vortex-revisor revisa de novo]
vortex-revisor: Spec ✅. Qualidade da task: Aprovada.
...
[Despacha revisão final de toda a branch com vortex-revisor]
...
**vs. execução manual:**
- Subagentes seguem TDD naturalmente (ver `vortex-tdd`).
...
- **`vortex-brainstorm`** — explora intenção e requisitos antes de existir
  qualquer plano.
- **`vortex-plano`** — produz o plano de implementação que esta skill executa.
- **`vortex-verificacao`** — aplique a mentalidade de evidência antes de
  afirmar antes de qualquer alegação de "pronto", tanto sua quanto a dos
  subagentes.
- **Agents `vortex-implementador`, `vortex-dev-backend`, `vortex-dev-frontend`,
  `vortex-dev-dados`, `vortex-dev-devops`** — quem implementa a task, conforme o
  campo `Agente` do plano. Todos seguem `vortex-tdd`.
- **Agent `vortex-revisor`** — revisor por task (spec + qualidade) e da
