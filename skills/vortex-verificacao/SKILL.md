...
# Vortex — Verificação Antes de Declarar Concluído
...
## Como isso se conecta ao fluxo de subagentes (`vortex-subagentes`)

Ao usar `vortex-subagentes`, cada gate desse fluxo é um ponto de aplicação
desta skill:

- O relatório "CONCLUIDO" de um `vortex-implementador` não é evidência — é uma
  ...
  `vortex-revisor`.
- Os dois vereditos do `vortex-revisor` (spec + qualidade) só valem se ele de
