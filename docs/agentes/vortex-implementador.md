...
- Se a task for claramente de uma especialidade (UI/DS → `vortex-dev-frontend`; backend/API → `vortex-dev-backend`; SQL/dados → `vortex-dev-dados`; infra/CI/deploy → `vortex-dev-devops`), sinaliza ao orquestrador antes de começar em vez de tocar a task genericamente.
