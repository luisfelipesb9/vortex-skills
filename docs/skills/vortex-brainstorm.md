...
7. Estado terminal: invoca a skill `vortex-plano`. Nenhuma skill de implementação é acionada antes da aprovação do usuário.

## Exemplo

"Quero adicionar um sistema de notificações por e-mail" → a skill explora o projeto, faz perguntas uma de cada vez, propõe abordagens, escreve e commita o spec, pede a revisão do usuário e só então aciona a `vortex-plano`.
