...
comportamento é `jet-brainstorm` (ou `/jet-feature`); bug é despacho direto ao especialista; entrega
de agência começa no `jet-gestor-projetos`.

Essa fronteira é deliberada. Uma skill que responde "como uso isso" tem descrição perigosamente
próxima da que responde "quero criar isso" — se ela roubar a invocação da `jet-brainstorm`, o
HARD-GATE de design deixa de ser aplicado.

## Exemplo

"O Claude travou dizendo `[JET/ledger] Bloqueado: redespacho da Task 3`, o que eu faço?" → explica
que o ledger marca a task como concluída, manda conferir com `/jet-ledger status`, e diz que reabrir
