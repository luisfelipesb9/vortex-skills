...
A `jet-subagentes` chama isso de "o erro mais caro observado": um controlador
...
def despacho(prompt, subagent_type="jet-implementador", tool_name="Agent", raiz=None):
    return {
        "tool_name": tool_name,
...
class TestMatriz(unittest.TestCase):
    ...
    def test_02_ledger_ausente_libera(self):
        ...
        self.assertEqual(decidir(despacho("leia .jet/sdd/tasks/task-3-brief.md", raiz=r)), "allow")

    def test_03_redespacho_de_task_concluida_e_negado(self):
        self.assertEqual(decidir(despacho("leia .jet/sdd/tasks/task-3-brief.md")), "deny")

    def test_04_nivel_warn_pergunta(self):
        self.assertEqual(decidir(despacho("leia .jet/sdd/tasks/task-3-brief.md"), "warn"), "ask")

    def test_05_nivel_off_libera(self):
        self.assertEqual(decidir(despacho("leia .jet/sdd/tasks/task-3-brief.md"), "off"), "allow")

    def test_06_task_nao_concluida_passa(self):
        self.assertEqual(decidir(despacho("leia .jet/sdd/tasks/task-4-brief.md")), "allow")

    def test_07_revisor_sobre_task_concluida_passa(self):
        """Re-revisao e legitima: o gate e sobre implementacao, nao sobre leitura."""
        self.assertEqual(decidir(despacho("revise task-3", subagent_type="jet-revisor")), "allow")

    def test_08_especialista_tambem_e_implementador(self):
        self.assertEqual(
            decidir(despacho("leia .jet/sdd/tasks/task-3-brief.md", subagent_type="jet-dev-frontend")),
            "deny")
    ...
    def test_10_brief_tem_precedencia_sobre_a_prosa(self):
        ...
        p = ("leia .jet/sdd/tasks/task-5-brief.md — ela consome o que a Task 3 produziu "
    ...
    def test_11_fronteira_de_numero(self):
        ...
        self.assertEqual(decidir(despacho("leia .jet/sdd/tasks/task-10-brief.md")), "allow")
...
class TestMensagem(unittest.TestCase):
    def test_segue_o_contrato_de_saida(self):
        bruto = gate_despacho.responder(despacho("leia .jet/sdd/tasks/task-3-brief.md"), "block")
        motivo = json.loads(bruto)["hookSpecificOutput"]["permissionDecisionReason"]
        self.assertTrue(motivo.startswith("[JET/ledger]"), motivo)
