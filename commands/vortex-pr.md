...
when_to_use: Quando a execucao de um plano terminou e a branch precisa virar PR — tipicamente depois de /vortex-executar, que para antes deste passo por rodar em contexto forkado.
---

Abra o PR de entrega desta branch. Siga a seção **Fechamento — abrir o PR** da skill `vortex-subagentes` — este comando é a reentrada manual do mesmo procedimento, não um caminho paralelo.
...
3. `gh auth status` ok.
4. Ledger (`.vortex/sdd/progress.md`) sem task não marcada. Se houver, **avise quais e pergunte** antes de seguir — PR parcial é uma decisão legítima, mas tem que ser consciente.
5. A revisão final de branch aconteceu. Se não houver registro dela, despache o `vortex-revisor` com o range completo antes de abrir.
