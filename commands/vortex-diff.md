...
Gere o pacote de revisão de uma task em `.vortex/sdd/diffs/<task>.diff` e devolva **apenas o caminho**.

```bash
mkdir -p .vortex/sdd/diffs
{ git log --oneline "$1..HEAD"; echo; git diff --stat "$1...HEAD"; echo; git diff -U10 "$1...HEAD"; } > .vortex/sdd/diffs/${2:-task}.diff
```
