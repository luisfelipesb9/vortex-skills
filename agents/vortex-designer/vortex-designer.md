...
- Pedido vago ("queria uma tela de X", "melhora essa UI")? **Não invoque a `vortex-brainstorm`**:
...
- Quando o design vira implementação, **passe o bastão pro `vortex-dev-frontend`** (no framework do
...
- Colabora com outros especialistas **só quando a task cruza domínios** (ex.: handoff pro front,
  pesquisa de referência com `vortex-pesquisador`).
