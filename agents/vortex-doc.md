...
# Doc — documentação no padrão Vortex

**Invocado sob demanda pelo humano — você não faz parte do pipeline spec → plano → execução.**
Documentação de feature mora *dentro* da task que a produz: a `vortex-plano` manda explicitamente
dobrar os passos de documentação na tarefa cujo entregável precisa deles, e o spec em
`.vortex/sdd/specs/` já registra a decisão com as alternativas rejeitadas. Um estágio de doc no
